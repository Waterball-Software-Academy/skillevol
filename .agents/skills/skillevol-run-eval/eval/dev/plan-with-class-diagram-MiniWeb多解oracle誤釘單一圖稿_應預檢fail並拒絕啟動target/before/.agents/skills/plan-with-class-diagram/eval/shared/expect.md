...
- 每個 unit 的 `after/` 必須包含規劃階段產出的 Mermaid 類別圖檔。合法 file diff 僅限新增或更新 `docs/architecture/*.class.mmd`；`src/`、需求文件、測試與設定檔必須語意等同 `before/`。
...
決定性 invariant：
- 跑完至 ASK(implement?) 時，終態 fs 必須語意等同同 unit 的 `after/`。
- `before/` 到 `after/` 的 file outcome 必須只包含 `docs/architecture/*.class.mmd` 類別圖檔；不得新增、刪除或修改 `src/`、測試、設定檔或需求文件。
