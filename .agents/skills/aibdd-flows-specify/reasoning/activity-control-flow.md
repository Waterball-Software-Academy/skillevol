...
4. fork／parallel／join：僅當 `$P` 明示可並行、互不阻塞、且完成條件為「全部完成才續行」時，使用 fork 拆支線、在匯攏處使用 join；每一並行支線上的 Action 節點仍是 api-wise 業務 Action（顆粒度依 `aibdd-flows-specify/rules/apiwise-granularity.md`，其 binds_feature 為下游落檔之契約路徑）；不得把單一業務意圖之內部技術鏈新拆成 parallel 節點。
...
7. 寫回：通過上一項檢查後，將圖形與註解整理為該 flow 對應之 `activity_analysis.activity` 推理包（交由後續 /aibdd-form-activity DELEGATE 步驟落地至該 flow 的 `.activity`）。跨 flow 的 Action 若因 `aibdd-flows-specify/rules/activity-diagram-granularity.md` 必須分檔，不得硬併入本檔。
